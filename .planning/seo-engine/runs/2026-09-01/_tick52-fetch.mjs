/**
 * TICK52 cited-only fetch. Per-alias esearch (never OR-joined).
 * Amycretin oral/SC + Mazdutide GLORY-1/GLORY-2/Hsia.
 */
...
function printAbstracts(xml) {
  ...
  for (const c of parts) {
    ...
    const titleMatch = need.every((w) => titleLower.includes(w.toLowerCase()));
...
const aliases = [
  ["amycretin", `"amycretin"`],
  ["mazdutide", `"mazdutide"`],
  ["GLORY-1 title", `"GLORY-1"[Title]`],
  ["GLORY-2 title", `"GLORY-2"[Title]`],
  ["amycretin AND mazdutide", `"amycretin" AND "mazdutide"`],
...
const known = "40550229,40550231,40421736,42251595,42628555";
...
const ncts = ["NCT05369390", "NCT06064006", "NCT05607680", "NCT06164873", "NCT06124807"];
...
for (const name of ["amycretin", "mazdutide"]) {
  await sleep(200);
  const fda = await get(
