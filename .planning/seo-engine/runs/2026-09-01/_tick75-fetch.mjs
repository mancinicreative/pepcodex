/**
 * TICK75 per-alias fetch. Not OR-joined. Title-match before quote.
 * 5-Amino-1MQ + retatrutide phase 2 / TRIUMPH design / TRANSCEND-T2D-1.
 */
...
async function main() {
  console.log("TICK75 fetch start", new Date().toISOString());
  ...
  const aliasTerms = [
    ...
    '"5-amino-1-methylquinolinium"',
    '"retatrutide"',
    '"5-Amino-1MQ" AND "retatrutide"',
    '"5-amino-1-methylquinolinium" AND "retatrutide"',
    '"TRIUMPH-1"[Title]',
    '"TRIUMPH-1" retatrutide weight',
    "retatrutide[Title] AND 28.7",
    "retatrutide[Title] AND 28.3",
    "37366315[uid]",
    "41090431[uid]",
    "42250575[uid]",
    "42608321[uid]",
  ...
  const retaIds = [
    ...new Set([
      ...
      "42608321",
      "38858523",
      ...(searchHits["37366315[uid]"]?.ids || []),
      ...(searchHits["41090431[uid]"]?.ids || []),
      ...(searchHits["42250575[uid]"]?.ids || []),
      ...(searchHits["42608321[uid]"]?.ids || []),
      ...(searchHits["38858523[uid]"]?.ids || []),
      ...(searchHits["NCT04881760"]?.ids || []),
      ...(searchHits["NCT06354660"]?.ids || []),
      ...(searchHits['"TRIUMPH-1" retatrutide weight']?.ids || []),
      ...(searchHits['"TRIUMPH-1"[Title]']?.ids || []),
    ]),
  ];
  ...
  console.log("\n=== retatrutide known esummary ===");
  const retaSum = await esummary(retaIds);
  await sleep(400);
  console.log("\n=== TITLE_MATCH retatrutide ===");
  for (const r of retaSum.rows || []) {
    const ok = titleMatch(r.title, ["retatrutide"]);
    console.log(r.pmid + " | " + (ok ? "TITLE_MATCH" : "MISS") + " | " + r.title);
  }

  const h2hIds = [
    ...new Set([
      ...(searchHits['"5-Amino-1MQ" AND "retatrutide"']?.ids || []),
      ...(searchHits['"5-amino-1-methylquinolinium" AND "retatrutide"']?.ids || []),
    ]),
  ];
  ...
  for (const g of ["5-amino-1mq", "5-amino-1-methylquinolinium", "retatrutide"]) {
    await openfda(g);
    await sleep(200);
  }
