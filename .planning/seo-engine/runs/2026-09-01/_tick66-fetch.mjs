/**
 * TICK66 per-alias fetch. Not OR-joined. Title-match before quote.
 * AOD-9604 + orforglipron. Human RCT / Phase 3 searches included.
 */
...
async function main() {
  console.log("TICK66 fetch start", new Date().toISOString());
  ...
  const aliasTerms = [
    ...
    '"ATTAIN-1"',
    '"Foundayo"',
    '"AOD-9604" AND "orforglipron"',
    '"AOD9604" AND "orforglipron"',
  ...
  const orfIds = [
    ...new Set([
      ...(searchHits['"orforglipron"']?.ids || []).slice(0, 12),
      ...(searchHits['"LY3502970"']?.ids || []),
  ...
  for (const r of orfSum.rows || []) {
    const ok = titleMatch(r.title, orfNeedles);
    console.log(r.pmid + (ok ? " | TITLE_MATCH | " : " | MISS | ") + r.title);
  }

  const h2hIds = [
    ...new Set([
      ...(searchHits['"AOD-9604" AND "orforglipron"']?.ids || []),
      ...(searchHits['"AOD9604" AND "orforglipron"']?.ids || []),
    ]),
  ];
  ...
  for (const term of ["AOD-9604", "AOD9604", "AOD 9604", "orforglipron", "LY3502970", "ATTAIN-1"]) {
    await ctgov(term);
    await sleep(250);
  }
  ...
  for (const [field, value] of [
    ...
    ["openfda.generic_name", "orforglipron"],
    ["openfda.brand_name", "Foundayo"],
  ]) {
