/**
 * TICK67 per-alias fetch. Not OR-joined. Title-match before quote.
 * AOD-9604 + cagrilintide. Human RCT / Phase 3 searches included.
 */
...
async function main() {
  console.log("TICK67 fetch start", new Date().toISOString());
  ...
  const aliasTerms = [
    ...
    '"CagriSema"',
    '"REDEFINE 1"',
    '"AOD-9604" AND "cagrilintide"',
    '"AOD9604" AND "cagrilintide"',
    ...
    '"cagrilintide" AND monotherapy AND obesity',
    "15134286[uid]",
    "11146367[uid]",
    "11673763[uid]",
    "16625817[uid]",
  ...
  const cagIds = [
    ...new Set([
      ...(searchHits['"cagrilintide"']?.ids || []).slice(0, 12),
      ...(searchHits['"CagriSema"']?.ids || []).slice(0, 8),
      ...(searchHits['"REDEFINE 1"']?.ids || []),
      ...(searchHits['"cagrilintide" AND monotherapy AND obesity']?.ids || []),
      ...(searchHits["34798060[uid]"]?.ids || []),
      ...(searchHits["40544433[uid]"]?.ids || []),
      ...(searchHits["41328546[uid]"]?.ids || []),
    ]),
  ];
  ...
  console.log("\n=== cagrilintide candidate esummary ===");
  const cagSum = await esummary(cagIds);
  await sleep(400);
  console.log("\n=== TITLE_MATCH cagrilintide ===");
  const cagNeedles = ["cagrilintide", "cagrisema", "redefine"];
  for (const r of cagSum.rows || []) {
    const ok = titleMatch(r.title, cagNeedles);
    console.log(r.pmid + (ok ? " | TITLE_MATCH | " : " | MISS | ") + r.title);
  }

  const h2hIds = [
    ...new Set([
      ...(searchHits['"AOD-9604" AND "cagrilintide"']?.ids || []),
      ...(searchHits['"AOD9604" AND "cagrilintide"']?.ids || []),
    ]),
  ];
  ...
  for (const term of ["AOD-9604", "AOD9604", "AOD 9604", "cagrilintide", "CagriSema", "REDEFINE 1"]) {
    await ctgov(term);
    await sleep(250);
  }
  ...
  for (const [field, value] of [
    ...
    ["openfda.generic_name", "cagrilintide"],
  ]) {
