/**
 * TICK63 per-alias fetch. Not OR-joined. Title-match before quote.
...
async function main() {
  console.log("TICK63 fetch start", new Date().toISOString());
  ...
  const aliasTerms = [
    ...
    '"SCALE" AND liraglutide AND obesity',
  ...
  const toFetch = [...new Set(["35013352", "39067875", "33645410", "26132939", "27295427", ...aminoIds, ...h2hIds])];
  ...
  for (const b of ["victoza", "saxenda"]) {
