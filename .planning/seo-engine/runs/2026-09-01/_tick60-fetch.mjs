/**
 * TICK60 per-alias fetch. Not OR-joined. Title-match before quote.
...
async function main() {
  console.log("TICK60 fetch start", new Date().toISOString());
  ...
  const ctMatch = (ctSum.rows || []).filter((r) => titleMatch(r.title, ctNeedles));
  console.log(ctMatch.map((r) => r.pmid + " | TITLE_MATCH | " + r.title).join("\n") || "(none)");
  console.log("\n=== ct titles not matched ===");
  for (const r of ctSum.rows || []) {
    if (!titleMatch(r.title, ctNeedles)) console.log(r.pmid + " | MISS | " + r.title);
  }
  ...
  const toFetch = [
    ...new Set(["35013352", "41319798", "39067875", "33645410", "34176426", ...aminoIds, ...ctIds, ...h2hIds]),
  ].slice(0, 12);
