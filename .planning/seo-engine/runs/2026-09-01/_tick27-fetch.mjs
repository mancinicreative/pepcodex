...
const known = "40550229,40550231,33567185,37952131";
...
const ncts = ["NCT05369390", "NCT06064006", "NCT03548935", "NCT03574597"];
...
for (const name of ["amycretin", "semaglutide"]) {
  await sleep(200);
  const fda = await get(
...
for (const brand of ["OZEMPIC", "WEGOVY"]) {
  await sleep(200);
  const fda = await get(
