...
async function main() {
  ...
  const known = [
    { id: "40550229", expect: /amycretin/i, label: "ORAL" },
    { id: "40550231", expect: /amycretin/i, label: "SC" },
    { id: "35013352", expect: /methylquinolinium|NNMT|5-amino/i, label: "DIO" },
    { id: "39067875", expect: /methylquinolinium|NNMT|5-amino/i, label: "CAF" },
    { id: "33645410", expect: /methylquinolinium|NNMT|5-amino/i, label: "HELA" },
  ];
  ...
  const ctQueries = ["5-Amino-1MQ", "5-amino-1-methylquinolinium", "NNMT 5-amino", "amycretin"];
  for (const q of ctQueries) {
    await sleep(350);
    const url = `https://clinicaltrials.gov/api/v2/studies?query.term=${encodeURIComponent(q)}&pageSize=8`;
    const { status, text } = await get(url);
  ...
  for (const name of ["5-amino-1mq", "5-amino-1-methylquinolinium", "amycretin"]) {
    await sleep(200);
    const fda = await get(
