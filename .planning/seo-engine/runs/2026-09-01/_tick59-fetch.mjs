/**
 * TICK59 per-alias fetch. Not OR-joined. Title-match before quote.
 * 5-Amino-1MQ + CagriSema (REDEFINE 1/2).
 */
...
async function main() {
  console.log("TICK59 fetch start", new Date().toISOString());
  ...
  const aliasTerms = [
    ...
    '"5-Amino-1MQ" AND "CagriSema"',
    '"5-amino-1-methylquinolinium" AND "CagriSema"',
  ...
  const cagriIds = [
    ...new Set([
      "40544433",
      "40544432",
      ...(searchHits['"CagriSema"']?.ids || []),
      ...(searchHits["40544433[uid]"]?.ids || []),
      ...(searchHits["40544432[uid]"]?.ids || []),
      ...(searchHits["NCT05567796"]?.ids || []),
      ...(searchHits["NCT05394519"]?.ids || []),
    ]),
  ];
  ...
  console.log("\n=== CagriSema known + esearch esummary ===");
  const cagriSum = await esummary(cagriIds.slice(0, 20));
  await sleep(400);
  console.log("\n=== TITLE_MATCH CagriSema / cagrilintide+semaglutide ===");
  const cagriNeedles = ["cagrisema", "cagrilintide"];
  for (const r of cagriSum.rows || []) {
    const m = titleMatch(r.title, cagriNeedles);
    console.log(r.pmid + (m ? " | TITLE_MATCH | " : " | MISS | ") + r.title);
  }

  const h2hIds = [
    ...new Set([
      ...(searchHits['"5-Amino-1MQ" AND "CagriSema"']?.ids || []),
      ...(searchHits['"5-amino-1-methylquinolinium" AND "CagriSema"']?.ids || []),
  ...
  const toFetch = [
    ...new Set(["35013352", "39067875", "33645410", "40544433", "40544432", ...aminoIds, ...h2hIds]),
  ].slice(0, 8);
  ...
  for (const term of ["5-Amino-1MQ", "5-amino-1-methylquinolinium", "NNMT 5-amino", "CagriSema"]) {
    await ctgov(term);
    await sleep(250);
  }
  ...
  for (const g of ["5-amino-1mq", "5-amino-1-methylquinolinium", "cagrisema", "cagrilintide", "semaglutide"]) {
    await openfda(g);
    await sleep(200);
  }
