...
const aliases = ["pemvidutide", "ALT-801"];
...
const known = "41237796,41113119,35658024,37385275";
...
const ncts = ["NCT05295875", "NCT05989711"];
for (const nct of ncts) {
  await sleep(400);
  const url = `https://clinicaltrials.gov/api/v2/studies/${nct}?fields=NCTId,BriefTitle,OfficialTitle,OverallStatus,Phase,EnrollmentInfo,HasResults,PrimaryOutcomeMeasures,SecondaryOutcomeMeasures,StartDateStruct,CompletionDateStruct,PrimaryCompletionDateStruct,Conditions,Interventions,ArmGroups`;
  const { status, text } = await get(url);
...
const fda = await get(
  "https://api.fda.gov/drug/drugsfda.json?search=openfda.generic_name:pemvidutide+OR+openfda.brand_name:pemvidutide+OR+products.brand_name:pemvidutide&limit=5"
);
console.log("===== OPENFDA PEMVIDUTIDE STATUS", fda.status, "=====");
console.log(fda.text.slice(0, 1500));
console.log("");
