...
const known = "40550229,40550231,40549887";
...
const ctQueries = [
  ["query MariTide", "https://clinicaltrials.gov/api/v2/studies?query.term=MariTide&pageSize=20"],
  ["query maridebart", "https://clinicaltrials.gov/api/v2/studies?query.term=maridebart&pageSize=20"],
  ["query AMG 133", "https://clinicaltrials.gov/api/v2/studies?query.term=AMG%20133&pageSize=20"],
  ["query MARITIME", "https://clinicaltrials.gov/api/v2/studies?query.term=MARITIME&pageSize=20"],
];
for (const [name, url] of ctQueries) {
  ...
  try {
    ...
    console.log("totalCount", j.totalCount, "returned", rows.length);
...
for (const name of ["amycretin", "maritide", "maridebart", "cafraglutide", "amg-133", "amg133"]) {
  await sleep(200);
  const fda = await get(
