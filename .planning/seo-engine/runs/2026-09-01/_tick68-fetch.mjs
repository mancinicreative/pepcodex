/**
 * TICK68 per-alias fetch. Not OR-joined. Title-match before quote.
 * AOD-9604 + CagriSema (REDEFINE 1/2).
 */
...
async function main() {
  console.log("TICK68 fetch start", new Date().toISOString());
  ...
  const aliasTerms = [
    ...
    '"REDEFINE 1"',
    '"AOD-9604" AND "CagriSema"',
    '"AOD9604" AND "CagriSema"',
  ...
  const cagriIds = [
    ...new Set([
      "40544433",
      "40544432",
      ...(searchHits['"CagriSema"']?.ids || []).slice(0, 12),
  ...
  console.log("\n=== CagriSema candidate esummary ===");
  const cagriSum = await esummary(cagriIds.slice(0, 20));
  await sleep(400);
  console.log("\n=== TITLE_MATCH CagriSema / cagrilintide ===");
  const cagriNeedles = ["cagrisema", "cagrilintide"];
  for (const r of cagriSum.rows || []) {
    const ok = titleMatch(r.title, cagriNeedles);
    console.log(r.pmid + (ok ? " | TITLE_MATCH | " : " | MISS | ") + r.title);
  }

  const h2hIds = [
    ...new Set([
      ...(searchHits['"AOD-9604" AND "CagriSema"']?.ids || []),
      ...(searchHits['"AOD9604" AND "CagriSema"']?.ids || []),
  ...
  for (const term of ["AOD-9604", "AOD9604", "AOD 9604", "CagriSema"]) {
    await ctgov(term);
    await sleep(250);
  }
  ...
  for (const [field, value] of [
    ...
    ["openfda.generic_name", "cagrisema"],
